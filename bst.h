...
/**
* A templated class for a Node in a search tree. This represents
* a node in a normal
* binary search tree, but can also be extended in the future for other 
* kinds of search trees,
* such as Red Black trees, Splay trees, and AVL trees. You do NOT need 
* to implement any
...
public:
    ...
    // These functions are virtual so that any derived classes, 
    // such as the
    // AVLNode, can be overwritten to return AVLNode pointers. 
    // This is one
    // of the many advantages to using getters/setters instead of 
    // public data
    // in a struct.
...
*/
template<typename Key, typename Value>
Node<Key, Value>::Node(const Key& key, const Value& value, Node<Key, 
    Value>* parent)
    : mItem(key, value)
    , mParent(parent)
    , mLeft(NULL)
...
public:
    ...
    // A virtual insert function lets future derivations of this 
    //class implement
...
public:
    ...
    //Node<Key, Value>* head_; //or is mRoot supposed to mean head?
};
...
* inserting.  Implementing this will help you test your iterator,
* but is not necessary: if you
* don't implement it, then you can put your entire insert
* implementation in avlbst.h
*/
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::insert(const std::pair<Key, 
    Value>& keyValuePair)
{
...
template<typename Key, typename Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::successor(Node<Key, Value>* x)
{
    Node<Key, Value>* temp;
    temp = x;
    ...
    {
        temp = x->getParent();
        while (temp != NULL && temp->getParent()->getRight() == temp)
        {
            temp = x->getParent();
        }
        return temp;
